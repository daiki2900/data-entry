...
def print_data():
    if terms_var.get() == "Accepted":
        ...
        if fname and lname:
            ...
            print(f"Gender: {gender}, Age: {age}, Nationality: {nationality}")
            print(
                f"Registeration Status: {registration_status}, Number of courses: {courses}, Num of semesters: {semesters}")
            # storing data in an excel file
            filepath = "/home/daiki/Desktop/tkinter-workspace/data-entry-env/data-entry/data.xlsx"            
            # if the file does not exist create the file and give it a heading
            if not os.path.exists(filepath):
                workbook = openpyxl.Workbook()
                sheet = workbook.active
                heading = ["First Name", "Last Name", "Gender", "Age",
            ...
            sheet = workbook.active
            record = [fname, lname, gender, age, nationality,
                      courses, semesters, registration_status]
            sheet.append(record)
            workbook.save(filepath)
